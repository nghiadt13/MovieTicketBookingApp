...
@Tag(
        name = "Movies",
        description = "Movie catalogue endpoints: list, paged, get, create, update, soft delete."
)
class MovieController(private val movieService: MovieService) {
    ...
    @Operation(
            summary = "List movies",
            description =
                    "Returns movies filtered by optional status. If status is omitted and activeOnly=true (default) returns only active movies; otherwise returns all."
    )
    @ApiResponses(
            value =
                    [
    ...
    fun getAllMovies(
            @Parameter(description = "Filter by movie status")
            @RequestParam(required = false)
            status: MovieStatus?,
            @Parameter(description = "Return only active movies (soft-delete hidden)")
    ...
    @Operation(
            ...
            description =
                    "Standard Spring pagination: `page` (0-based), `size`, `sort` (e.g., `createdAt,desc`). Hydrates genres and formats per page efficiently."
    )
    ...
    @Operation(
            ...
            description =
                    "Returns a single active movie by id. Soft-deleted movies are not returned."
    )
    @ApiResponses(
            value =
                    [
                            ...
                            ApiResponse(
                                    responseCode = "404",
                                    description = "Movie not found or has been deleted",
                                    content =
                                            [
                                                    Content(
                                                            schema =
    ...
    @Operation(
            ...
            description = "Creates a new movie and associates provided genres/formats by ids."
    )
    @ApiResponses(
            value =
                    [
    ...
    @Operation(
            ...
            description = "Updates mutable fields and optionally re-associates genres/formats."
    )
    @ApiResponses(
            value =
                    [
    ...
    @Operation(
            ...
            description = "Marks the movie as inactive; data remains for auditing and history."
    )
    @ApiResponses(
            value =
                    [
